...
## ポジショニング: AI Agent実行型VTuber

Kurage AI VTuberは、ただ会話するアバターでも、配信でコメントに反応するだけのAITuberでもありません。自然文の業務依頼を受け取り、背後の`kdeck`へAI Agent Taskとして渡し、記事公開、動画生成、Kurage投稿、URL報告、Git commitまで、実際の成果物を返すことを目指した **AI Agent実行型VTuber** です。

多くのAI VTuberは、会話、記憶、音声、Live2D、配信リアクション、エンタメ性に重心があります。一方、多くのAI Agent製品は、チャット、IDE、ブラウザ、ワークフロー管理画面からタスクを実行します。Kurage AI VTuberは、この2つを接続します。VTuberが、実務AI Agentの受付・進行役・結果報告役になります。

これは「世界初級」を狙えるプロダクトコンセプトです。AIアバター、MCP、ブラウザ操作、AI Agentそのものが新しいという主張ではありません。独自性は、VTuber人格と業務Agent実行基盤を一体化している点です。

```text
VTuber人格
  -> 自然文の業務依頼
  -> kdeck Agent Task実行
  -> 記事・動画・投稿・commitなどの成果物
  -> VTuber画面で結果報告
```

目標は、VTuberを「話すAIキャラクター」から **仕事を進めるAIキャラクター** へ進化させることです。
...
kvtuberのデモ動画では、偽物・やっつけ・中身のない代用品を作らない。

- 実際の `/admin`、`/viewer`、`/viewer?broadcast=1` を使う
- 管理画面をクリックしたと言うなら、本当に管理画面でクリックする
- セミナー配信と言うなら、LLMで展開された中身のある番組内容を使う
- セミナー中は本物のviewer音声を使い、デモ解説ナレーションを被せない
- 前後の解説ナレーションは、Kurage/Horizon/HyperFrames品質の見た目と構成で作る
- 口パク位置、音声、字幕、viewer配置を確認してから公開する

実録できない場合は、偽物でごまかさず、先にワークフローを直す。
