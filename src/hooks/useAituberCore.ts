  ...
  useEffect(() => {
    ...
    const core = new AITuberOnAirCore({
      ...
      chatOptions: {
        systemPrompt:
          systemPrompt ||
          'あなたは「Kurage AI Navigator」というクラゲ型AITuberです。経営者や開発者に、AI活用・動画生成・業務自動化をやさしく整理して説明します。短く、明るく、聞き取りやすい日本語で返答してください。視聴者コメントには名前を呼びながら自然に返してください。',
      },
      ...
      if (options.displayUserMessage !== false) {
        setMessages((prev) => [
          ...prev,
          {
            id: createMessageId(),
            role: 'user',
            content: options.displayText?.trim() || text.trim(),
            timestamp: Date.now(),
          },
        ]);
      }
