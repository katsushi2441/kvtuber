...
const DEFAULT_OPENAI_COMPATIBLE_ENDPOINT =
  `${window.location.origin}/ollama/v1/chat/completions`;
const DEFAULT_OPENAI_COMPATIBLE_TTS_ENDPOINT =
  `${window.location.origin}/kurage-tts/v1/audio/speech`;
...
function getDefaultSettings(): AppSettings {
  return {
    ...
    tts: {
      ...
      openAiCompatibleModel: 'kurage-edge-tts',
...
function loadSettings(): AppSettings {
  try {
    ...
    if (raw) {
      ...
      const savedTts: Partial<AppSettings['tts']> = saved.tts || {};
      const savedTtsEndpoint = savedTts.openAiCompatibleApiUrl;
      const shouldUseDefaultTts =
        !saved.tts ||
        (savedTts.engine === 'none' &&
          (!savedTtsEndpoint ||
            savedTtsEndpoint === 'http://localhost:8880/v1/audio/speech')) ||
        savedTtsEndpoint === 'http://localhost:8880/v1/audio/speech';
      return {
        llm: {
          ...defaults.llm,
          ...
          endpoint:
            !savedEndpoint ||
            savedEndpoint === 'http://localhost:11434/v1/chat/completions' ||
            savedEndpoint ===
              `${window.location.protocol}//${window.location.hostname}:11434/v1/chat/completions`
              ? defaults.llm.endpoint
              : savedEndpoint,
        ...
        tts: shouldUseDefaultTts
          ? defaults.tts
          : { ...defaults.tts, ...saved.tts },
